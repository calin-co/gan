...
class CatsDogsDataProvider(DataProvider):
    ...
    def __init__(self, batch_size=100, max_num_batches=-1,
        ...
        data_path = "data_sets/cats_dogs.npz"
        loaded = np.load(data_path)
        inputs, targets = loaded['arr'], loaded['label']
        self.num_classes = 2
        super(CatsDogsDataProvider, self).__init__(
            inputs, targets, batch_size, max_num_batches, shuffle_order, rng)
        
    def next(self):
        try:
            inputs_batch, targets_batch = super(CatsDogsDataProvider, self).next()
